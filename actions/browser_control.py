...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".rencora_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".rencora_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using RENCORA profile")
                rencora = str(Path.home() / ".rencora_profiles" / "firefox_rencora")
                Path(rencora).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(rencora, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".rencora_profiles" / "safari")
        ...
        rencora_profile = str(Path.home() / ".rencora_profiles" / self.browser_name)
        Path(rencora_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with RENCORA profile: {rencora_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(rencora_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser]  Launched [{label}] with RENCORA profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "rencora_screenshot.png")
