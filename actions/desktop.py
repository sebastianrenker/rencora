...
_FORBIDDEN = ("__", "import", "eval", "exec", "compile", "globals", "locals",
              "getattr", "setattr", "vars", "subprocess", "ctypes", "winreg",
              "os.system", "popen", "open(")

# Bloße Namen, die im generierten Code nicht vorkommen duerfen (Zugriff auf
# Builtins/Module fuer Code-Ausfuehrung, Import oder Prozessstart).
_FORBIDDEN_NAMES = frozenset({
    "eval", "exec", "compile", "__import__", "getattr", "setattr", "delattr",
    "vars", "globals", "locals", "open", "input", "breakpoint", "memoryview",
    "os", "sys", "subprocess", "ctypes", "winreg", "importlib", "builtins",
})
# Attributnamen, die einen Sandbox-Ausbruch/Prozessstart bedeuten. Legitime
# pathlib-/shutil-Methoden (read_text, write_text, open, copy2 ...) bleiben erlaubt.
_FORBIDDEN_ATTRS = frozenset({
    "system", "popen", "spawn", "getattr", "setattr", "delattr",
})


def _is_code_safe(code: str) -> bool:
    """Zweistufig: schneller Substring-Vorfilter, dann strukturelle AST-Pruefung.
    Die AST-Pruefung faengt Importe, Dunder-Zugriffe und gefaehrliche Aufrufe
    unabhaengig von Formatierung/Schreibweise ab (robuster als reine Textsuche).
    Nicht parsebarer Code gilt als unsicher."""
    low = code.lower()
...
def _execute_generated_code(code: str, player=None) -> str:
    ...
    if not _is_code_safe(code):
        return "This action was blocked for safety."
    ...
    try:
        exec(compile(code, "<rencora_desktop>", "exec"), sandbox)
