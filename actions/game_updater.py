...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        if not win:
            print("[GameUpdater]  Steam penceresi bulunamadı")
            return False
        ...
        if not colorful.any():
            print("[GameUpdater]  Avatar rengi bulunamadı — tahminle tıklanıyor")
        ...
        print(f"[GameUpdater]  Eksik kütüphane: {e}")
        return False
    except Exception as e:
        print(f"[GameUpdater]  Profil tespiti başarısız: {e}")
        return False
...
def _handle_steam_profile_selection() -> bool:
    ...
    try:
        ...
        if not is_small and white_pixels <= 100:
            print("[GameUpdater] ℹ Profil dialogu yok — Steam zaten giriş yapmış")
            return False
    except ImportError:
        pass
    except Exception:
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    try:
        ...
        print(f"[GameUpdater]  pywinauto başarısız: {e}")
        return _handle_install_dialog_pyautogui(game_name, best_drive)
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    print("[GameUpdater]  Steam başlatılamadı")
    return False
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        print(f"[GameUpdater]  AppID arama başarısız: {e}")
...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.rencora.gameupdater.plist"
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.rencora.gameupdater</string>
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", "RENCORA_GameUpdater", "/F"],
            capture_output=True, text=True
        )
        return ("Scheduled update cancelled."
                if result.returncode == 0 else "No scheduled update found.")
    if is_mac():
        plist_path = Path.home() / "Library" / "LaunchAgents" / "com.rencora.gameupdater.plist"
    ...
    try:
        existing = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        lines    = [l for l in existing.stdout.splitlines()
                    if "RENCORA_GameUpdater" not in l]
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", "RENCORA_GameUpdater", "/FO", "LIST"],
            capture_output=True, text=True
        )
        if result.returncode != 0:
