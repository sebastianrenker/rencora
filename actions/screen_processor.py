...
_SYSTEM_PROMPT = (
    "You are RENCORA, an advanced AI assistant. "
...
class _VisionSession:
    ...
    def set_broadcast_fn(self, fn) -> None:
        """Registriert eine async-Callback-Funktion fn(dict), die bei jeder
        fertigen Antwort zusätzlich zum lokalen player.write_log() aufgerufen
        wird — z.B. DashboardServer.broadcast(), damit Handy/iPad-Clients die
        Objekterkennungs-Antwort ebenfalls live sehen/vorlesen können."""
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript:
                        ...
                        if full:
                            if self._player:
                                self._player.write_log(f"RENCORA: {full}")
                            print(f"[Vision]  {full}")
                            if self._broadcast_fn:
                                try:
                                    coro = self._broadcast_fn(
                                        {"type": "log", "speaker": "rencora", "text": full}
                                    )
                                    if coro is not None:
                                        asyncio.create_task(coro)
                                except Exception as e:
                                    print(f"[Vision]   Broadcast error: {e}")
                    transcript = []
...
def register_broadcast(fn) -> None:
    """Wird einmalig beim Programmstart aufgerufen (siehe main.py), damit
    Vision-Antworten — egal ob durch PC-Kamera oder Handy-Foto ausgelöst —
    auch an alle per Dashboard/App verbundenen Geräte gesendet werden."""
    _session.set_broadcast_fn(fn)


def analyze_external_image(
    image_bytes: bytes,
    mime_type:   str,
    user_text:   str,
    player=None,
) -> bool:
    """Wie screen_process(), aber für Bilder, die NICHT von der lokalen PC-
    Kamera/dem Screen stammen, sondern z.B. von der RENCORA Handy/iPad-App
    hochgeladen wurden. Nutzt exakt dieselbe Gemini-Live-Vision-Session wie
    die PC-Objekterkennung, damit Verhalten und Antwortqualität identisch
    sind — nur die Bildquelle unterscheidet sich.
    """
    user_text = (user_text or "Was siehst du? Beschreibe kurz, was das Objekt ist und wofür es verwendet wird.").strip()

    if not image_bytes:
        print("[Vision]   Kein Bild übergeben — abgebrochen")
        return False

    try:
        _ensure_session(player=player)
    except Exception as e:
        print(f"[Vision]  Session konnte nicht gestartet werden: {e}")
        return False

    try:
        image_bytes, mime_type = _compress(image_bytes, source_format="JPEG" if "jpeg" in mime_type else "PNG")
    except Exception as e:
        print(f"[Vision]   Komprimierung fehlgeschlagen, sende Original: {e}")

    print(f"[Vision]  Externes Bild (App): {len(image_bytes):,} bytes — '{user_text[:80]}'")
    _session.analyze(image_bytes, mime_type, user_text)
    return True
